...
import { getReportingOverview, alertNonLoggers } from '../../api/reporting';
...
export default function ReportingOverviewPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
      <h2 className="text-xl font-bold">Reporting Overview</h2>
      ...
      {data && !loading && (
        <>
              ...
              {data.athletes.map((a) => (
                <div key={a.user_id} className="bg-white border rounded-lg px-3 py-2 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {a.group_name && (
                      <p className="text-xs text-gray-400 truncate">{a.group_name}</p>
                    )}
                  </div>

                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold">
                      {a.days_logged}/{a.total_days}
                    </p>
                    <p className="text-xs text-gray-400">
                      {Math.round(a.response_rate * 100)}%
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 space-y-2">
            ...
            {alertResult && (
              <p className="text-xs text-amber-900">{alertResult}</p>
            )}
          </div>
        </>
      )}
